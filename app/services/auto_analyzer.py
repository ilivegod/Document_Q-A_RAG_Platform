"""Post-upload project analysis: requirements extraction + technology stack generation."""
import logging
import uuid
...
from app.services.technology_stack import generate_project_stack
...
async def _set_analysis_status(
    ...
    *,
    ...
    technology_generated: bool | None = None,
) -> None:
    ...
    if technology_generated is not None:
        project.technology_generated = technology_generated
    if not analyzing:
...
async def run_project_analysis(
    ...
    """Extract requirements and seed the technology stack for a project."""
    ...
    if requirements_ok:
        try:
            created = await generate_project_stack(db, user_id, project_id)
            technology_ok = len(created) > 0
            logger.info(
                "Project %s: %d technology item(s) seeded",
                project_id,
                len(created),
            )
        except HTTPException as e:
            logger.warning(
                "Project %s: technology stack generation skipped: %s",
                project_id,
                e.detail,
            )
        except Exception as e:
            logger.error(
                "Project %s: technology stack generation failed: %s",
                project_id,
                e,
                exc_info=True,
            )
    ...
    await _set_analysis_status(
        ...
        technology_generated=technology_ok,
    )
